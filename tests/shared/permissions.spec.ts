			...
			it('should escape environment variables and arguments', function () {
				expect(
					permissions.createLaunchScript(
						'C:\\Users\\Alice & Bob\'s Laptop\\"what"\\vingInstaller',
						['"a Laser"', 'arg1', "'&/ ^ \\", '" $ % *'],
						{
							key: 'value',
							key2: ' " \' ^ & = + $ % / \\',
					...
					`chcp 65001${os.EOL}` +
						...
						`set "key2= " ' ^ & = + $ % / \\"${os.EOL}` +
						`set "key3=8"${os.EOL}` +
						`"C:\\Users\\Alice & Bob's Laptop\\\\"what\\"\\vingInstaller" "\\"a Laser\\"" "arg1" "'&/ ^ \\" "\\" $ % *"`,
				);
			});
		});
		...
		for (const platform of ['linux', 'darwin']) {
			describe(`given ${platform}`, function () {
				...
				it('should escape environment variables and arguments', function () {
					expect(
						permissions.createLaunchScript(
							'/home/Alice & Bob\'s Laptop/"what"/vingInstaller',
							['arg1', "'&/ ^ \\", '" $ % *'],
							{
								key: 'value',
								key2: ' " \' ^ & = + $ % / \\',
						...
						`export key='value'${os.EOL}` +
							...
							`'/home/Alice & Bob'\\''s Laptop/"what"/vingInstaller' 'arg1' ''\\''&/ ^ \\' '" $ % *'`,
					);
				});
			});
		}
